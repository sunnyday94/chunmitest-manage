    ...
    $.post("querytempadverbypagebeanajax.htm", { tempId: $(tempId).val(),CSRFToken:$(CSRFToken).val(),pageSize:pageSize,pageNo:pageNo,adverType:151,atId:atId},
        function(data){
            ...
            for(var i=0;i<data.pb.list.length;i++){
                ...
                if(data.pb.list[i].temp5 == 0 || data.pb.list[i].temp5==null){
                    str+=" <td>首页广告</td>";
                }else if(data.pb.list[i].temp5 == 1){
                    str+=" <td>商品列表页广告</td>";
                }else if(data.pb.list[i].temp5 == 2){
                    str+=" <td>搜索列表页广告</td>";
                }else if(data.pb.list[i].temp5 == 3){
                    str+=" <td>资讯页广告</td>";
                }else if(data.pb.list[i].temp5 == 4){
                    str+=" <td>其他。。。</td>";
                }else if(data.pb.list[i].temp5 == 5){
                    str+=" <td>团购页广告</td>";
                }else if(data.pb.list[i].temp5 == 6){
                    str+=" <td>抢购页广告</td>";
...
function doByAjax(pageNo) {
    ...
    $.get("queryProductForCoupon.htm?CSRFToken="+$(CSRFToken).val()+proHtml+"&pageNo="+pageNo+"&pageSize=5&flag=1",
        ...
        function(data){
            if(data != "") {
                ...
                for(var i=0;i<data.list.length;i++){
                    ...
                    str +=' <td>'+data.list[i].thirdName+' </td>';
...
function updateTrademark(id){
    $.post("showchannelstoreytrademarkajax.htm", { trademarkId: id,CSRFToken:$(CSRFToken).val()},
        function(data) {
            ...
            $("#trademarkBrand option[value='"+data.trademarkName+"']").attr("selected", true);
            $("#trademarkBrand").select2();
            $("#trademarBrandValue").val(data.logoSrc);
            $("#trademarkBrandImg").attr("src",data.logoSrc);
            if(data.userStatus==1){
                $("#trademarkStatus1").prop("checked","checked");
            }else{
