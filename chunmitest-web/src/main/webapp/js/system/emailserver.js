...
$(function() {
	...
	$('.form_value').click(function() {
		...
		if (!$('.form_btns').is(':visible')) {
			...
			$('.form_btns').show();
			$('.form_btns').css({
				'left' : formItem.parent().position().left + (formItem.next().find(".form-control")
								.width() == null ? formItem
								.next().width()
								: formItem.next().find(".form-control").width())+ 65 + 'px',
	...
	$('.form_sure').click(function() {
		...
		$.ajax({
			...
			success : function(data) {
				if ("radio" == attrType) {
					$(formItem).html(newValue == '0' ? '<span class="label label-default">否</span>'
