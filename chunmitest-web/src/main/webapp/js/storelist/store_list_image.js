 ...
 * 查询所有广告
 */
function storeliststreet(){
    location.href='storeliststreetimage.htm';
}
...
function delAdverById(adverIdUrl){
    showAjaxDeleteConfirmAlertByTemp(adverIdUrl,15);
}
...
function showTempBar(pageNo){
    ...
    $.post("queryTempBarByAjax.htm", { tempId: $(tempId).val(),CSRFToken:$(CSRFToken).val(),pageSize:pageSize,pageNo:pageNo},
        function(data){
            ...
            for(var i=0;i<data.pb.list.length;i++){
                var str="<tr>";
                str+=" <td><input  name='barId'  value="+data.pb.list[i].barId+" type='checkbox'></td>";
                str+=" <td>"+data.pb.list[i].barName+"</td>";
                str+=" <td>"+data.pb.list[i].barUrl +"</td>";
                str+=" <td>"+data.pb.list[i].barSort +"</td>";
                if(data.pb.list[i].usedStatus==1){
                ...
                str+= '<td><div class="btn-group"><button type="button" class="btn btn-default" onclick="updateBar('+data.pb.list[i].barId+')">编辑</button>'+
                '<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">'+
                '<span class="caret"></span>'+
                '<span class="sr-only">Toggle Dropdown</span>'+
                '</button><ul class="dropdown-menu" role="menu"><li><a onclick="delBarById(\'deleteTempBarAjxs.htm?barId='+data.pb.list[i].barId+'\')" href="javascript:;">删除</a></li></ul></div></td>';
                str+="</tr>";
                $(barHtml).append(str);
...
function showAdverPage(atId,pageNo){
    ...
    $("#upAtId").val(atId);
    $("#isAdver").val(1);
    $("#up_storeyAdverTempId").val( $(tempId).val());
    var pageSize=10;
    if($("#showAdverOpetion").val()!=null){
        pageSize=$(showAdverOpetion).val();
    }
    $.post("querytempadverbypagebeanajax.htm", { tempId: $(tempId).val(),CSRFToken:$(CSRFToken).val(),pageSize:pageSize,pageNo:pageNo,adverType:151,atId:atId},
        function(data){
            $(adverHtml).html("");
            if(data.pb.list.length==0){
            ...
            for(var i=0;i<data.pb.list.length;i++){
                ...
                if(data.pb.list[i].tempVal == 0 || data.pb.list[i].tempVal==null){
                    ...
                    str+=" <td>抢购页广告</td>";
                }/*else if(data.pb.list[i].tempVal == 7){
                 str+=" <td>类型8</td>";
                 }else if(data.pb.list[i].tempVal == 8){
                 str+=" <td>类型9</td>";
                 }else if(data.pb.list[i].tempVal == 9){
                 str+=" <td>类型10</td>";
                 }*/
                ...
                str+= '<td><div class="btn-group"><button type="button" class="btn btn-default" onclick="showAdver('+data.pb.list[i].channelAdverId+')">1编辑</button>'+
                '<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">'+
                '<span class="caret"></span>'+
                '<span class="sr-only">Toggle Dropdown</span>'+
                '</button><ul class="dropdown-menu" role="menu"><li><a onclick="delAdverById(\'deletetempadverajax.htm?CSRFToken='+$(CSRFToken).val()+'&adverId='+data.pb.list[i].channelAdverId+'\')" href="javascript:;">1删除</a></li></ul></div></td>';
                str+="</tr>";
    ...
    $.post("querymegawizardbypagebeanbyajax.htm", { tempId: $(tempId).val(),CSRFToken:$(CSRFToken).val(),pageSize:pageSize,pageNo:pageNo},
        function(data){
            ...
            for(var i=0;i<data.pb.list.length;i++){
                ...
                str+= 'megawizardName='+ data.pb.list[i].megawizardName+' msgSort='+data.pb.list[i].sort+' onclick="updateMsg(this,'+data.pb.list[i].id+')">2编辑</button>'+
                '<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">'+
                '<span class="caret"></span>'+
                '<span class="sr-only">Toggle Dropdown</span>'+
...
function showHotSearch(pageNo){
    ...
    $.post("queryhotpagesizeajax.htm", { tempId: $(tempId).val(),CSRFToken:$(CSRFToken).val(),pageSize:pageSize,pageNo:pageNo},
        function(data){
            $(hotHtml).html("");
            for(var i=0;i<data.pb.list.length;i++){
                var str="<tr>";
                ...
                str+= '<td><div class="btn-group"><button type="button" class="btn btn-default" key-word="'+data.pb.list[i].keyword+'" onclick="updateHotSearch(this,'+data.pb.list[i].sort +','+data.pb.list[i].hotSearchId +')">3编辑</button>'+
                '<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">'+
                '<span class="caret"></span>'+
                '<span class="sr-only">Toggle Dropdown</span>'+
    ...
    $.post("querychannelgoodsbypagebeanajax.htm", { tempId: $(tempId).val(),CSRFToken:$(CSRFToken).val(),pageSize:pageSize,pageNo:pageNo},
        ...
        function(data){
            ...
            for(var i=0;i<data.pb.list.length;i++){
                ...
                str+=" <td>"+data.pb.list[i].sort +"</td>";
                str+= '<td><div class="btn-group"><button type="button" class="btn btn-default" onclick="updateStorey('+data.pb.list[i].channelStoreyGoodsproductId+')">4编辑</button>'+
                '<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">'+
                '<span class="caret"></span>'+
                '<span class="sr-only">Toggle Dropdown</span>'+
...
function doByAjax(pageNo) {
    var productNo = $("#searchText").val();
    var proHtml = "";
    if(productNo != "") {
        proHtml="&productNo="+productNo;
    }else{
        proHtml="";
    }
    if(pageNo==null){
...
function showTempStorey(pageNo) {
    ...
    $.post("querytempstoreybypagebeanajax.htm", {
            tempId: $(tempId).val(),
        ...
        function (data) {
            ...
            for (var i = 0; i < data.pb.list.length; i++) {
                ...
                str += '  <li><a href="javascript:;" onclick="showStoreyList(' + data.pb.list[i].channelStoreyId + ')">设置楼层商品</a></li>';
...
function showChannelStoreyGoods(pageNo){
    ...
    $.post("querychannelstoreygoodsbypagebeanajax.htm", { tempId: $(tempId).val(),CSRFToken:$(CSRFToken).val(),pageSize:pageSize,pageNo:pageNo ,storeyId:$("#storeyId").val()},
...
function updateChannelStorey(channelStoreyGoodsproductId){
    $.post("showchannelgoodsajax.htm?CSRFToken="+$(CSRFToken).val(), { channelStoreyGoodsproductId:channelStoreyGoodsproductId },
        function(data){
            ...
            $(up_channelStoreyGoodsproductId1).val(channelStoreyGoodsproductId);
...
function subFloorStorey(){
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data == 1) {
                if($(up_channelStoreyGoodsproductId1).val()!=null&&$(up_channelStoreyGoodsproductId1).val()!=""){
                    showTipAlert("修改成功！！");
                }else{
                    showTipAlert("添加成功！！");
                }
                ...
                if($(up_channelStoreyGoodsproductId1).val()!=""){
                    showTipAlert("修改失败！！");
                }else{
                    showTipAlert("添加失败！！");
                }
    ...
    $.post("querychannelstoreyadverbypagebeanajax.htm", { tempId: $(tempId).val(),CSRFToken:$(CSRFToken).val(),pageSize:pageSize,pageNo:pageNo,storeyId:$("#storeyAdverId").val()},
        function(data){
            ...
            for(var i=0;i<data.pb.list.length;i++){
                ...
                str+= '<td><div class="btn-group"><button type="button" class="btn btn-default" onclick="showAdver('+data.pb.list[i].channelAdverId+')">2编辑</button>'+
                '<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">'+
                '<span class="caret"></span>'+
                '<span class="sr-only">Toggle Dropdown</span>'+
                '</button><ul class="dropdown-menu" role="menu"><li><a onclick="delFloorAdverById(\'deletetempadverajax.htm?CSRFToken='+$(CSRFToken).val()+'&adverId='+data.pb.list[i].channelAdverId+'\')" href="javascript:;">2删除</a></li></ul></div></td>';
                str+="</tr>";
